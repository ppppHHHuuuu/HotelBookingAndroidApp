...
public class CancelActiveTripTest {
    @Rule()
    public ActivityScenarioRule<MainActivity> activityActivityScenarioRule = new ActivityScenarioRule<>(MainActivity.class);

    @Test
    public void cancelActiveTrip() throws InterruptedException {
        sleep(2000);
        onView(withId(R.id.trips))
                .perform(click());
        sleep(2000);
        onView(withId(R.id.recycleView)).perform(
                RecyclerViewActions.actionOnItemAtPosition(0, MyViewAction.clickChildViewWithId(R.id.buttonCancel)));
        sleep(2000);
    }
}
